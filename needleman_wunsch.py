...
def menu(args):
    parser = argparse.ArgumentParser(formatter_class = argparse.RawDescriptionHelpFormatter,
                                     description = "Implementation of the Needleman-Wunsch algorithm",
...
class NeedlemanWunsch:
    ...
    def needleman_wunsch(self, seq1, seq2):
        ...
        last_box = matrix[m][n]
        score = last_box[self.param_maximum]
    ...
    def matrix_format(self, matrix, arr_seq1, arr_seq2):
        ...
        for index, row in enumerate(matrix_sum, start = 1):
            if index == 1:
                head_row = ['-', '-'.rjust(nchars, ' ')]
                first_row = ['-', '0'.rjust(nchars, ' ')]
                for idx, cell in enumerate(row, start = 1):
                    head_row.append(arr_seq1[idx - 1].rjust(nchars, ' '))
                    first_row.append(str(self.GAP_PENALTY * idx).rjust(nchars, ' '))
                matrix_full.append(head_row)
                matrix_full.append(first_row)
            rows = [arr_seq2[index - 1], str(self.GAP_PENALTY * index).rjust(nchars, ' ')]
            for cell in row:
...
def main(args):
    try:
        ...
        oalig.show_print("################## Needleman-Wunsch Algorithm ##################", [oalig.LOG_FILE])
        oalig.show_print("################################################################", [oalig.LOG_FILE])
        ...
        alignment1, alignment2, score, matrix = oalig.needleman_wunsch(oalig.SEQUENCE1, oalig.SEQUENCE2)
        ...
        app = odash.run_standalone_app('Needleman-Wunsch Algorithm')
